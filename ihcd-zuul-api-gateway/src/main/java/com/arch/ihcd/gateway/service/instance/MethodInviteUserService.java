...
@Slf4j
@Service
public class MethodInviteUserService implements IMethodInviteUserService{
	...
	@Override
	public User inviteMethodUserRole(MethodUserRoleAssignRequest request, String loginUserId) {
		/*Integer active = 1;
		...
		if(user == null || instanceUserRoleXRefRepository.findByInstIdAndUserIdAndStatus(request.getInstId(), user.getId(), true) == null){
			...
			log.info("<<MethodInviteUserService>>:<<inviteMethodUserRole>> Created/Updated user and associated to instance role");
		}
		request.setUserId(user.getId());
		log.info(">>>>MethodInviteUserService.inviteMethodUserRole<<<<calling master data service");
		userInviteServiceProxy.assignMethodUserRole(request, loginUserId);
		log.info(">>>>MethodInviteUserService.inviteMethodUserRole<<<< Proxy call completed");
		return user;
	}
